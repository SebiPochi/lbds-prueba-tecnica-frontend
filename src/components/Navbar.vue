...
<template>
  <nav class="w-full h-16 flex flex-row justify-between content-center px-8 bg-amber-200">
    <ul class="w-1/3 flex content-center justify-evenly">
      <li v-if="userStore.currentUser.type !=-1" class="flex content-center"> 
        <p class="self-center">{{ userStore.currentUser.type ? "Organizador" : "Borracho" }}</p>
      </li>
      <li v-else class="flex content-center"> 
        <p class="self-center">Visitante</p>
      </li>
    </ul>
    <div v-if="userStore.currentUser.type ==-1" class="w-fit flex gap-6">
      <RouterLink class="self-center rounded-sm p-2 bg-amber-50" to="/">Registrarse</RouterLink>
